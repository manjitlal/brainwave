<?php
	...
	if (isset($_POST['Regd_delete'])) {
		...
		if(!empty($reg)){
			$querydelete="delete from round_1_answer where regd='".$reg."' ";
			if(mysqli_query($con,$querydelete)){
				echo '<script type="text/javascript">
						alert("Answer Deleted");
					</script>';

				$querydelete2="delete from round_2_answer where regd='".$reg."'";
				if(mysqli_query($con,$querydelete2)){
					// echo '<script type="text/javascript">
					...
					echo '<script type="text/javascript">
						alert("2nd Round Deletetion Failed..");
					</script>';
				}

				$querydelete3="delete from scoreboard where regd='".$reg."'";
				if(mysqli_query($con,$querydelete3)){
					// echo '<script type="text/javascript">
					// 	alert("Deletion Failed.");
					// </script>';
				}
					...
					echo '<script type="text/javascript">
						alert("Scoreboard Deletetion Failed..");
					</script>';
				}
			}
			else{
...
<body>
	<div>
		<table class="table table-striped table-hover">
			...
			<tbody>
				<?php
					$query = "SELECT * FROM `round_1_answer`";
					...
					while($row = mysqli_fetch_assoc($query_result)) {
						...
						echo '<tr>';
							...
							echo '<td>'.$row["Regd"].'</td>';
							$queryQ = "SELECT `Question` FROM `round_1_question` WHERE `R1_id` = '".$row['Q_id']."'";
							$queryQ_result = mysqli_query($con, $queryQ);
							$rowQ = mysqli_fetch_assoc($queryQ_result);
							echo '<td>'.$rowQ["Question"].'</td>';
							echo '<td>'.$row['Answer'].'</td>';
							echo '<td>'.$row['Time'].'</td>';							
							echo '<form action="round_1_answer_view.php" method="POST">';
							echo '<input type="hidden" name="Regd" value="'.$row["Regd"].'" />';
							$queryS = "SELECT `Round_1` FROM `scoreboard` WHERE `Regd` = '".$row['Regd']."'";
							$queryS_result = mysqli_query($con, $queryS);
							$rowS = mysqli_fetch_assoc($queryS_result);
							echo '<td><input type="text" name="marks" value="'.$rowS["Round_1"].'" /></td>';
